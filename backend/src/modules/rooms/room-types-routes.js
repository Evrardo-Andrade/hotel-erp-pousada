...
async function ensureUniqueName(nome, currentId = null) {
  ...
  if (result.rows.length) {
    throw new AppError("Ja existe um tipo de quarto com este nome.", 409);
  }
}
...
router.get("/", authorizePermission("rooms.read"), async (request, response) => {
  ...
  const result = await query(
    `SELECT id, nome, descricao, diaria_base, ativo, created_at, updated_at
     FROM tipos_quarto
...
router.post("/", authorizePermission("rooms.manage"), auditAction("create", "tipos_quarto"), async (request, response) => {
    ...
    const restored = await query(
      `UPDATE tipos_quarto
       SET nome = $2,
           ...
           updated_at = NOW()
       WHERE id = $1
       RETURNING id, nome, descricao, diaria_base, ativo, created_at, updated_at`,
  ...
  const result = await query(
    `INSERT INTO tipos_quarto (nome, descricao, diaria_base, ativo)
     VALUES ($1, $2, 0, true)
     RETURNING id, nome, descricao, diaria_base, ativo, created_at, updated_at`,
...
router.put("/:id", authorizePermission("rooms.manage"), auditAction("update", "tipos_quarto"), async (request, response) => {
  ...
  if (!result.rows.length) {
    throw new AppError("Tipo de quarto nao encontrado.", 404);
  }

  return response.json(result.rows[0]);
});

router.patch("/:id/active", authorizePermission("rooms.manage"), auditAction("update_active", "tipos_quarto"), async (request, response) => {
...
router.delete("/:id", authorizePermission("rooms.manage"), auditAction("delete", "tipos_quarto"), async (request, response) => {
  ...
  if (usage.rows.length) {
    throw new AppError("Nao e possivel excluir um tipo de quarto em uso.", 409);
  }
